...
class DialGauge(Gauge):
    ...
    def __init__(self, name, min, max, r, length, rotation = 200, delay = 1):
        ...
        #시작 좌표(바늘의 회전 축)
        self.orix, self.oriy = self.r, self.r
        ...
        #데이터가 저장될 영역
        self.image = [" "*self.W for _ in range(self.H)]                    # string array 눈에 보여질 존재
    ...
    def show(self):
        ...
        rad = math.radians((self.base_r + self.rotation * self.get_per())%360) #라디안 각도
        ...
        for i in range(self.length*rep):
            x, y = self.orix + i/rep*math.cos(rad), self.oriy + i/rep*math.sin(rad);
        ...
        for i in range(0, self.H, 2):
            for j in range(0, self.W, 1):
                if i+1 == self.H:
                    self.image += dot2[self.idea[i][j]]
                else :
                    self.image += dot2[self.idea[i][j]*2+self.idea[i+1][j]]
            self.image +=" \n"
